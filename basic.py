...
if dicom_zip:
    ...
    if nrrd_file:
        ...
        if seg is not None:
            ...
            img_overlay = overlay_segmentation(img, seg_slice_resized, alpha=0.4, color=[0, 255, 0])

    # Display side by side
    col1, col2 = st.columns(2)

    with col1:
        st.image(resize_img(img), caption=f"Slice {slice_idx} — Raw", use_column_width=True)

    with col2:
        if img_overlay is not None:
            st.image(resize_img(img_overlay), caption=f"Slice {slice_idx} — Overlay", use_column_width=True)
        else:
            st.write("No segmentation uploaded")
